...
public class HangmanGame {
    public static void main() throws IOException {
        ...
        while(guessesLeft > 0 && playing){
            ...
            String playResponse = IO.readln("Would you like to play again? (Y/N): ");
            if (playResponse == null) {           // user hit EOF/cancel
                System.out.println("Goodbye!");
                break;
            }

            String ans = playResponse.trim().toUpperCase();

            if (ans.equals("Y")) {
                playAgain = true;                 // loop continues
            } else if (ans.equals("N")) {
                playAgain = false;                // loop ends
            } else {
                System.out.println("Invalid response. Ending game.");
                playAgain = false;                // end on invalid
            }
        }while(playAgain);
